...
retarget() {
    ...
    # Fit the SMPL shape that matches the Taks_T1 kinematics
    echo "Running grad_fit_Taks_T1_shape.py..."
    python scripts/data_process/grad_fit_Taks_T1_shape.py || return 1
    ...
    # Retarget the AMASS dataset based on the corresponding keypoints between fitted SMPL shape and Taks_T1
    echo "Running grad_fit_Taks_T1.py on $amass_dir..."
    python scripts/data_process/grad_fit_Taks_T1.py --amass_root "$amass_dir" || return 1
...
print_usage() {
    ...
    echo "Retarget AMASS dataset to Taks_T1 robot."
    echo ""
    echo "Options:"
...
# Note: We create a new patch for Taks_T1 if needed, or use the existing one
# apply_patch $HUMAN2HUMANOID_DIR/scripts/data_process/grad_fit_Taks_T1.py third_party/grad_fit_Taks_T1.patch || exit 1
...
if [ $? -ne 0 ]; then
    ...
    echo "Motion retargeting finished. You should find the retargeted dataset at $HUMAN2HUMANOID_DIR/data/Taks_T1/amass_all.pkl."
fi
